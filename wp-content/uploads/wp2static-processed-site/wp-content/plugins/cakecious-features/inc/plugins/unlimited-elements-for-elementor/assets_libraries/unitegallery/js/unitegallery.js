...
if(isExternalLoad!==true){g_ugFunctions.loadJs("cdnjs.cloudflare.com/ajax/libs/mediaelement/2.18.1/mediaelement.min.js",true);g_ugFunctions.loadCss("cdnjs.cloudflare.com/ajax/libs/mediaelement/2.18.1/mediaelementplayer.min.css",true);}
...
window.ugArrGalleriesObjects={};window.ugArrGalleriesObjects[galleryID]=objGallery;var api=new UG_API(objGallery);return(api);}
...
t.continuePlaying();}}
function onKeyPress_isStopEventScanAllPageGalleries(){trace(window.ugArrGalleriesObjects);jQuery.each(window.ugArrGalleriesObjects,function(){});}
function onKeyPress(event){var obj=jQuery(event.target);if(obj.is("textarea")||obj.is("select")||obj.is("input"))
return(true);var keyCode=(event.charCode)?event.charCode:((event.keyCode)?event.keyCode:((event.which)?event.which:0));var wasAction=false;switch(keyCode){case 39:var isInsideScreen=t.isGalleryInsideScreen();if(isInsideScreen==true){t.nextItem();wasAction=true;}
break;case 37:var isInsideScreen=t.isGalleryInsideScreen();if(isInsideScreen==true){t.prevItem();wasAction=true;}
...
this.isGalleryInsideScreen=function(){if(!g_objSlider)
...
this.isLightboxOpened=function(){if(!g_objLightbox)
return(false);}
