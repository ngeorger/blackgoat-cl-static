...
class Cakecious_Compatibility_Elementor {

	/**
	 * Singleton instance
	 *
	 * @var Cakecious_Compatibility_Elementor
	 */
	 ...
	 *
	 * @return Cakecious_Compatibility_Elementor
	 */
	...
	protected function __construct() {
		...
		add_filter( 'cakecious/dataset/page_settings/ignored_post_types', array( $this, 'disable_page_settings_on_elementor_template' ) );
		...
		add_action( 'wp_ajax_cakecious_apply_color_palette_to_elementor', array( $this, 'ajax_apply_color_palette_to_elementor' ) );
	}
	...
	public function add_compatibility_css() {
		$css = "\n/* Elementor Compatibility CSS */\n" . cakecious_minify_css_string( '.elementor-text-editor > *:last-child { margin-bottom: 0; }' );

		wp_add_inline_style( 'cakecious', trim( $css ) );
	}
	...
	public function add_theme_fonts_as_options_on_font_control( $fonts ) {
		...
		if ( class_exists( '\Elementor\Fonts' ) ) {
			foreach( cakecious_get_web_safe_fonts() as $font => $stack ) {
				$fonts[ $font ] = \Elementor\Fonts::SYSTEM;
			}

			foreach( cakecious_get_google_fonts() as $font => $stack ) {
	...
	public function remove_content_wrapper_on_page_templates( $template ) {
		...
		if ( false !== strpos( $template, '/elementor/' ) ) {
			if ( false !== strpos( $template, '/header-footer.php' ) ) {
				...
				add_filter( 'cakecious/frontend/show_content_wrapper', '__return_false' );
			}
		}
	...
	public function add_page_template_header_footer_wrapper() {
		?>
		<div id="content" class="site-content cakecious-section">
		...
		if ( 'elementor_library' === $post->post_type ) {
			...
			if ( ! empty( $terms ) ) {
				switch ( $terms[0] ) {
				 	case 'section':
				 	case 'page':
				 		$template = CAKECIOUS_INCLUDES_DIR . '/compatibilities/elementor/templates/single-elementor_library.php';
				 		break;
				}
			}
		}
	...
	public function register_customizer_settings( $wp_customize ) {
		...
		$wp_customize->add_control( new Cakecious_Customize_Control_Heading( $wp_customize, 'heading_color_palette_elementor', array(
			'section'     => $section,
			'settings'    => array(),
			'label'       => esc_html__( 'Elementor Integration', 'cakecious' ),
			'priority'    => 30,
		) ) );

		// Apply to Elementor color picker
		$wp_customize->add_control( new Cakecious_Customize_Control_Blank( $wp_customize, 'apply_color_palette_to_elementor', array(
			'section'     => $section,
			'settings'    => array(),
			'description' => '<button class="cakecious-apply-color-palette-to-elementor button button-secondary">' . esc_html__( 'Apply to Elementor color picker', 'cakecious' ) . '</button><br><br>' . esc_html__( 'Pressing this button will replace the existing Elementor color picker with the colors you defined above. You can still change and edit Elementor color picker like usual.', 'cakecious' ),
			'priority'    => 30,
		) ) );
	}
	...
	public function print_custom_scripts() {
		...
		<script type="text/javascript">
			(function( $ ) {
				...
				$( '#customize-controls' ).on( 'click', '.cakecious-apply-color-palette-to-elementor', function( e ) {
					e.preventDefault();
					...
					return $.ajax({
						...
						cache: false,
						data: {
							action: 'cakecious_apply_color_palette_to_elementor',
							_ajax_nonce: CakeciousAdminData.ajax_nonce,
						},
					})
					.done(function( response, status, XHR ) {
						if ( response.success ) {
							alert( '<?php esc_html_e( 'Color palette applied!', 'cakecious' ); ?>' );
						} else {
							alert( '<?php esc_html_e( 'Error!', 'cakecious' ); ?>' );
						}
	...
	public function ajax_apply_color_palette_to_elementor() {
		check_ajax_referer( 'cakecious', '_ajax_nonce' );
...
Cakecious_Compatibility_Elementor::instance();