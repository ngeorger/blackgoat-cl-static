...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Cakecious_Customize_Control_Slider' ) ) :
/**
 * Slider control class
 */
class Cakecious_Customize_Control_Slider extends Cakecious_Customize_Control {
	/**
	 * @var string
	 */
	public $type = 'cakecious-slider';
	...
	protected function content_template() {
		...
		<div class="customize-control-content">
			<# _.each( data.structures, function( setting_key, device ) { #>
				<div class="cakecious-slider-fieldset cakecious-row {{ data.responsive ? 'cakecious-responsive-fieldset' : '' }} {{ 'desktop' == device ? 'active' : '' }} {{ 'preview-' + device }}">
					<div class="cakecious-row-item">
						<input class="cakecious-slider-range" type="range" value="{{ data.inputs[ setting_key ].number }}" min="{{ data.units[ data.inputs[ setting_key ].unit ].min }}" max="{{ data.units[ data.inputs[ setting_key ].unit ].max }}" step="{{ data.units[ data.inputs[ setting_key ].unit ].step }}">
					</div>
					<div class="cakecious-row-item" style="flex: 0 0 50px;">
						<input class="cakecious-slider-input {{ data.hide_units ? '' : 'cakecious-input-with-unit' }}" type="number" value="{{ data.inputs[ setting_key ].number }}" min="{{ data.units[ data.inputs[ setting_key ].unit ].min }}" max="{{ data.units[ data.inputs[ setting_key ].unit ].max }}" step="{{ data.units[ data.inputs[ setting_key ].unit ].step }}">
					</div>
					<div class="cakecious-row-item" style="flex: 0 0 30px; {{ data.hide_units ? 'display: none;' : '' }}">
						<select class="cakecious-slider-unit cakecious-unit">
					...
					<input type="hidden" class="cakecious-slider-value" value="{{ data.inputs[ setting_key ].value }}" {{{ data.inputs[ setting_key ].__link }}}>
				</div>
			<# }); #>
		</div>
		<?php
...
$wp_customize->register_control_type( 'Cakecious_Customize_Control_Slider' );
endif;