...
function cakecious_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		$categories_list = get_the_category_list( esc_html__( ', ', 'cakecious' ) );
			printf( '<span class="cat-links2">' . esc_html__( 'Posted in %1$s', 'cakecious' ) . '</span>', $categories_list );

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'cakecious' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links2">' . esc_html__( 'Tagged %1$s', 'cakecious' ) . '</span>', $tags_list );
		}
	}
/*		edit_post_link(
		sprintf(
