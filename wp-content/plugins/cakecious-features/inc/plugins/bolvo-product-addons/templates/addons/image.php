...
<?php if ( empty( $required ) ) { ?>
	<a href="#" title="<?php echo esc_attr__( 'None', 'woocommerce-product-addons' ); ?>" class="blv-cust-p-addon-image-swatch" data-value="" data-price="">
...
<?php foreach ( $addon['options'] as $i => $option ) {
	...
	?>
		<a href="#" title="<?php echo esc_attr( $option['label'] . ' ' . $price_tip ); ?>" class="blv-cust-p-addon-image-swatch" data-value="<?php echo sanitize_title( $option['label'] ) . '-' . $loop; ?>" data-price="<?php echo esc_attr( '<span class="blv-cust-p-addon-image-swatch-price">' . wptexturize( $option['label'] ) . ' ' . $price_display . '</span>' ); ?>">
			<img src="<?php echo esc_url( $image_src[0] ? $image_src[0] : wc_placeholder_img_src() ); ?>" />
		</a>
<?php } ?>

<select class="blv-cust-p-addon-image-swatch-select blv-cust-p-addon-field" name="addon-<?php echo sanitize_title( $field_name ); ?>">
	<?php if ( empty( $required ) ) { ?>
