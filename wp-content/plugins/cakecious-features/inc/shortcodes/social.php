...
<iframe src="' . esc_url( $src_url ) . '" scrolling="no" frameborder="0" allowTransparency="true" style="border:none; overflow:hidden; width:' . esc_attr( $widthpx ) . '; height:' . esc_attr( $height ) . 'px;"></iframe>
    ...
    add_shortcode( 'cakecious_fblike', 'tt_temptt_sc_fblike_fn' );
}
...
if( !function_exists( 'tt_temptt_sc_twitter_fn' )) {
    function tt_temptt_sc_twitter_fn( $atts, $content = null ) {
        ...
        $output = '<div class="tt-sc-twitter ' . esc_attr( $float )  .' '. esc_attr( $class ) .'"><a href="' . esc_url( 'http://twitter.com/share' ) . '" class="twitter-share-button"' . $output . '>' . __( 'Tweet', 'cakecious' ) . '</a><script type="text/javascript" src="' . esc_url( 'http://platform.twitter.com/widgets.js' ) . '"></script></div>';

        return $output;

    } // End tt_shortcode_twitter()

    add_shortcode( 'cakecious_twitter', 'tt_temptt_sc_twitter_fn' );
}
...
if ( !function_exists( 'tt_temptt_sc_pin_fn' ) ) {
    function tt_temptt_sc_pin_fn( $atts, $content = null ) {
        ...
        $title = ( isset( $atts['title'] ) ) ? $atts['title'] : __( 'Pin This Post', 'cakecious' );
    ...
    add_shortcode( 'cakecious_pinterest', 'tt_temptt_sc_pin_fn' );
}